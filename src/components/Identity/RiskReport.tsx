...
export default function RiskReport({ results, loading }: any) {
  if (loading)
    return (
      <div className="flex flex-col items-center justify-center p-20 space-y-4">
        ...
        <p className="text-slate-500 font-bold text-[10px] animate-pulse uppercase tracking-[0.3em]">
    ...
    return (
      <div className="p-20 text-center border-2 border-dashed border-slate-800/50 rounded-3xl text-slate-600 bg-slate-900/10">
        ...
        <p className="text-sm font-medium italic">
          Awaiting security artifacts for analysis...
        </p>
      </div>
    );

  const rawVerdict = results.verdict || "INVALID";

  const verdict =
    rawVerdict === "INVALID_ENTITY"
      ? "INVALID"
      : rawVerdict === "AUTHENTIC"
        ? "REAL"
        : rawVerdict === "DIGITAL_ARTIFACT"
          ? "DIGITAL_SRC"
          : rawVerdict;

  const trust = Math.max(
    0,
    Math.min(100, Math.round(Number(results.trustScore ?? results.score ?? 0)))
  );

  const explicitRisk = Number(
    results.deepfakeRisk ??
      results.riskPercentage ??
      results.riskScore ??
      results.risk ??
      results.deepfakeScore
  );

  const risk =
    Number.isFinite(explicitRisk) && explicitRisk > 0
      ? Math.max(0, Math.min(100, Math.round(explicitRisk)))
      : verdict === "DEEPFAKE"
        ? Math.max(70, Math.min(100, Math.round(100 - trust)))
        : verdict === "REAL"
          ? Math.max(0, Math.min(30, Math.round(100 - trust)))
          : Math.max(0, Math.min(100, Math.round(100 - trust)));

  const category = results.category || "UNKNOWN";

  const reasoning =
    results.reasoning ||
    results.riskReasoning ||
    "Establishing neural fingerprint for subject analysis...";

  const captureSource =
    results.captureSource === "DIGITAL_ARTIFACT" || verdict === "DIGITAL_SRC"
      ? "SCREENSHOT"
      : verdict === "REAL"
        ? "DIRECT SENSOR"
        : verdict === "INVALID"
  ...
  const styles =
    VERDICT_REGISTRY[verdict as keyof typeof VERDICT_REGISTRY] ||
    VERDICT_REGISTRY.INVALID;

  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
      <div
        className={`relative overflow-hidden p-6 rounded-2xl border transition-colors duration-500 ${styles.bg} ${styles.border}`}
      >
        <div className="flex justify-between items-center relative z-10">
          <div>
            ...
            <h3
              className={`text-3xl font-black italic tracking-tighter ${styles.color}`}
            >
              {styles.label}
            </h3>
          </div>
          ...
          <div
            className={`h-14 w-14 rounded-2xl flex flex-col items-center justify-center border transition-all ${styles.border} ${styles.color} bg-white/5`}
          >
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <span
              className={`text-xs font-bold ${
                risk < 40 ? "text-emerald-400" : "text-rose-400"
              }`}
            >
              {risk}% Risk
            </span>
      ...
      <div className="bg-[#1e293b]/20 border border-slate-800 rounded-3xl p-6 backdrop-blur-sm">
        <div className="flex items-center justify-between mb-6 border-b border-slate-800 pb-4">
          ...
          <span className="text-[9px] bg-slate-950 text-slate-500 px-2 py-0.5 rounded italic font-mono uppercase tracking-tighter">
            Scan ID: #GAIA-{Math.floor(Math.random() * 99)}
          </span>
        </div>
        ...
        <ul className="space-y-4">
          <CheckItem
            ...
            status={results.subjectCount > 0 && verdict !== "INVALID"}
            refId={results.subjectCount > 0 && verdict !== "INVALID" ? "BIOLOGICAL" : "NONE"}
          />

          <CheckItem
            label="Neural Artifact Mapping"
            status={!results.isSynthetic && verdict !== "DEEPFAKE"}
            refId={results.isSynthetic || verdict === "DEEPFAKE" ? "GAN_DETECTED" : "CLEAN"}
          />

          <CheckItem
            label="Source Integrity Check"
            status={verdict !== "DEEPFAKE" && verdict !== "INVALID"}
            refId={
              verdict === "DIGITAL_SRC"
                ? "SCREEN_CAP"
                : verdict === "INVALID"
      ...
      <span
        className={`text-[10px] font-mono px-2 py-1 rounded border ${
          status
            ...
            : "text-rose-500/60 bg-rose-500/5 border-rose-500/10"
        }`}
      >
        {refId}
      </span>
